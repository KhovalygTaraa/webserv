...
void Server::start() {
	...
	while(1) {
		...
		for(int i = 0; fds[i].fd != -2 || event != 0; i++)
		{
			if (fds[i].revents == POLLRDNORM)
			{
				fds[i].revents = 0;
				new_client = accept(fds[i].fd, NULL, NULL);
				std::cout << "\e[1;32m[new_connection]-\e[0m" << event << std::endl;
				if (new_client == -1) {
					throw std::strerror(errno);
				}
//				if (recv(new_client, &buffer, BUFFER_SIZE, 0) == -1) {
...
//				send(new_client, rec.c_str(), rec.length(), 0);
			...
			else if (fds[i].revents == POLLIN)
			{
				...
				send(fds[i].fd, rec.c_str(), rec.length(), 0);
				event--;
			}
		...
		std::cout << "\e[1;32m[OK]-\e[0m" << event << std::endl;
	}
//	if (check == -1)
